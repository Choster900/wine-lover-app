<template>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[30px]" data-aos="fade-up" data-aos-delay="100">
        <div v-for="(item, index) in blogData2" :key="index" class="group">
            <router-link to="/blog-details-v1"
                class="p-[10px] sm:p-[15px] bg-[#F3F3F3] transition-all duration-300 group-hover:bg-opacity-0 overflow-hidden block">
                <img class="duration-200 transform scale-100 group-hover:scale-110 w-full" :src="item.image"
                    alt="blog-card">
            </router-link>
            <div class="pt-5 md:pt-6 md:px-4 text-center">
                ...
                <div class="mt-5 md:mt-6">
                    <router-link
                        ...
                        to="/blog-details-v1">Read More</router-link>
                </div>
            </div>
        </div>
    </div>
